...
def info(update: Update, context: CallbackContext):
    ...
    text = (
        ...
        f"╰( ･ ᗜ ･ )➝ ID: <code>{user.id}</code>\n"
        f"╰( ･ ᗜ ･ )➝ First Name: {html.escape(user.first_name)}"
    )

    if user.last_name:
        text += f"\n╰( ･ ᗜ ･ )➝ Last Name: {html.escape(user.last_name)}"

    if user.username:
        text += f"\n╰( ･ ᗜ ･ )➝ Username: @{html.escape(user.username)}"

    text += f"\n╰( ･ ᗜ ･ )➝ Permalink: {mention_html(user.id, 'link')}"

    if chat.type != "private" and user_id != bot.id:
        _stext = "\n╰( ･ ᗜ ･ )➝ Presence: <code>{}</code>"
